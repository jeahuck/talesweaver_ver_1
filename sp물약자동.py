...
# 설정
# ==============================
THRESHOLD = 1   # 템플릿 매칭 유사도 기준
VK_3 = 0x33         # '3' 키
VK_RETURN = 0x0D    # 엔터 키
VK_RIGHT = 0x27     # 오른쪽 방향 키
SKIP_CHK = False
lock = threading.Lock()
VK_F3 = 0x72
...
def select_img(fileList, hwnd, win_left, win_top):
    ...
    # click_client_coords(client_x - win_left, client_y - win_top, hwnd)
    # _last_click["pos"] = pos
...
def worker_2():
    ...
    fileList = [
        {'imgName': 'im/po/po.png', 'callBackKey': VK_F3},
    ]


    # 메인 루프: 모니터 한 번 캡처 -> 모든 템플릿 검사 -> 대기 -> 반복
    while True:
        changed = select_img(fileList, hwnd, 0, 0)
